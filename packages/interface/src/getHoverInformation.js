...
/*
export function _getHoverInformation(
  schema: GraphQLSchema,
  queryText: string,
  cursor: Position,
  contextToken?: ContextToken,
  ...
  const {fieldDef, type, parentType} = typeInfo;
  ...
  if (type) {
    if (type instanceof GraphQLNonNull) {
      typeName = `${type.ofType.name}!`; // '!' indicates non-nullable
      typeDescription = type.ofType.description || '';
    } else {
  ...
  if (parentType) {
    parentTypeName = String(parentType.name || '');
  }
  ...
  if (parentTypeName && fieldName) {
    formattedFieldName = `${parentTypeName}.${fieldName}`;
  } else if (fieldName) {
    formattedFieldName = `${fieldName}`;
  }

  let summary = '';
  if (formattedFieldName && typeName) {
    summary = `${formattedFieldName}: ${typeName}`;
  }
  ...
  return [
    ...
    },
    {
      ...
      value: fieldDescription
        ? `Field \`${formattedFieldName}\` — ${fieldDescription}`.trim()
        : '',
    },
    {
      kind: 'markdown',
      value: typeDescription
        ? `Type \`${typeName}\` — ${typeDescription}`.trim()
        : '',
    },
  ];
}
